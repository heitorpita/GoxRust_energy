...
# Compila os algoritmos de ordenacao em Rust e os executa sobre as listas .in
# medindo energia com o perf, gerando um csv por (algoritmo, tamanho, execucao).
#
# Uso: ./implementacao_rust.sh [opcoes]
#   -a, --algoritmo    "bolha_rs insertion_rs" | rust_bubble | todos  (padrao: todos)
...
#   -C, --sem-compilar nao recompila os fontes .rs antes de medir
...
# arrays paralelos: termo do csv, pasta de saida e fonte .rs de cada algoritmo
CSV_TERMS=("bolha_rs"        "insertion_rs"        "selection_rs"        "bolha_rs_better"  "insertion_rs_better"  "selection_rs_better")
FOLDERS_TERMS=("rust_bubble" "rust_insert"         "rust_selection"      "rust_bubble"      "rust_insert"          "rust_selection")
SOURCE_FILES=("bubble_sort_v1.rs" "insertion_sort_v1.rs" "selection_sort_v1.rs" "bubble_sort_v2.rs" "insertion_sort_v2.rs" "selection_sort_v2.rs")

PASTA_LISTAS="geracoes_listas"
SCRIPT_LISTAS="criacao_lista.sh"
PREFIXO_LISTA="lista"

PASTA_FONTES="rust_algs"
PASTA_BINARIOS="rust_bin"
COMPILADOR="rustc"
FLAGS_COMPILACAO="-O -C debuginfo=0"
...
if [ "$SEM_COMPILAR" -eq 0 ] && ! command -v "$COMPILADOR" > /dev/null; then
    echo "Erro: '$COMPILADOR' nao encontrado no PATH (use --sem-compilar se ja tem os binarios)"
    exit 1
fi
...
if [ "$ALGORITMO" = "todos" ]; then
    ...
    # aceita varios algoritmos ("bolha_rs bolha_rs_better") e tambem o nome da pasta
...
# nome do binario de cada fonte: bubble_sort_v1.rs -> rust_bin/bubble_sort_v1
binario_de() {
    local base
    base=$(basename "${SOURCE_FILES[$1]}" .rs)
    echo "$PASTA_BINARIOS/$base"
}


# compila os fontes dos algoritmos pedidos; so recompila o que esta desatualizado
if [ "$SEM_COMPILAR" -eq 0 ]; then
    echo "Compilando os fontes Rust..."

    mkdir -p "$PASTA_BINARIOS"

    compilados=0
    atualizados=0

    for indice in "${INDICES[@]}"; do
        fonte="$PASTA_FONTES/${SOURCE_FILES[$indice]}"
        binario=$(binario_de "$indice")

        if [ ! -f "$fonte" ]; then
            echo "Aviso: '$fonte' nao encontrado, pulando '${CSV_TERMS[$indice]}'"
            continue
        fi
        ...
        echo "-> $COMPILADOR $FLAGS_COMPILACAO $fonte"

        if ! $COMPILADOR $FLAGS_COMPILACAO -o "$binario" "$fonte"; then
            echo "   ERRO: falha ao compilar '$fonte'"
            exit 1
        fi
    ...
    echo "Binarios compilados: $compilados | ja atualizados: $atualizados"
    echo
fi
